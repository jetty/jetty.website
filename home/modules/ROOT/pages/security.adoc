= Jetty Security

== Reporting Security Issues

There are a couple of avenues for reporting security issues to the Jetty project. If the issue is directly related to Jetty itself, then reporting to the Jetty developers is encouraged. The most direct method is to mail mailto:security@jetty.org[security@jetty.org] or mailto:security@webtide.com[security@webtide.com]. We are flexible in how we work with reporters of security issues but reserve the right to act in the interests of the Jetty project in all circumstances.

If the issue is related to Eclipse or its Jetty integration, we would like to encourage you to reach out to mailto:security@eclipse.org[security@eclipse.org].

If the issue is related to integrations with Jetty, we are happy to work with you to identify the proper entity, and either of the approaches above is okay.

We prefer that security issues be reported directly to Jetty developers via email instead of GitHub issues since it has no facility to tag issues as private. We will actively delete issues that are opened in this way.

For more information on how we handle security issues, please refer to our https://github.com/jetty/jetty.project/blob/jetty-12.1.x/SECURITY.md[Security Policy].

== Jetty Security Reports

[cols="1,2,1,1,1"]
|===
|Published|ID|Severity|Impacted Versions|Fixed Versions

|Sep 8, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-12611[CVE-2026-12611]
|High
|\<=9.4.63, \<=10.0.31, \<=11.0.31, \<=12.0.37, \<=12.1.10
|12.0.38, 12.1.11 (9.4.64, 10.0.32, 11.0.32 see https://webtide.com/end-of-life/[details] for availability)

|Sep 8, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-19203[CVE-2026-19203]
|High
|\<=9.4.63, \<=10.0.31, \<=11.0.31, \<=12.0.37, \<=12.1.11
|12.0.38, 12.1.12 (9.4.64, 10.0.32, 11.0.32 see https://webtide.com/end-of-life/[details] for availability)

|Sep 7, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-19204[CVE-2026-19204]
|High
|\<=10.0.31, \<=11.0.31, \<=12.0.37, \<=12.1.11
|12.0.38, 12.1.12 (10.0.32, 11.0.32 see https://webtide.com/end-of-life/[details] for availability)

|Jul 13, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-7708[CVE-2024-7708]
|High
|\<=10.0.22, \<=11.0.22
|10.0.23, 11.0.23

|Jul 13, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-8384[CVE-2026-8384]
|Med
|\<=12.0.34, \<=12.1.8
|12.0.35, 12.1.9

|Jul 13, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-6790[CVE-2026-6790]
|Med
|\<=9.4.60, \<=10.0.28, \<=11.0.28, \<=12.0.34, \<=12.1.8
|12.0.35, 12.1.9 (9.4.61, 10.0.29, 11.0.29 see https://webtide.com/end-of-life/[details] for availability)

|Jul 13, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-10051[CVE-2026-10051]
|Med
|\<=12.0.35, \<=12.1.9
|12.0.36, 12.1.10

|Jul 13, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-10050[CVE-2026-10050]
|High
|\<=9.4.62, \<=10.0.30, \<=11.0.30, \<=12.0.35, \<=12.1.9
|12.0.36, 12.1.10 (9.4.63, 10.0.31, 11.0.31 see https://webtide.com/end-of-life/[details] for availability)

|Apr 11, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-5795[CVE-2026-5795]
|High
|\<=9.4.60, \<=10.0.28, \<=11.0.28, \<=12.0.33, \<=12.1.7
|12.0.34, 12.1.8 (9.4.61, 10.0.29, 11.0.29 see https://webtide.com/end-of-life/[details] for availability)

|Apr 11, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-2332[CVE-2026-2332]
|High
|\<=9.4.59, \<=10.0.27, \<=11.0.28, \<=12.0.32, \<=12.1.6
|12.0.33, 12.1.7 (9.4.60, 10.0.28, 11.0.29 see https://webtide.com/end-of-life/[details] for availability)

|Mar 5, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-1605[CVE-2026-1605]
|High
|\<=12.0.31, \<=12.1.5
|12.0.32, 12.1.6

|Mar 5, 2026
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2025-11143[CVE-2025-11143]
|Low
|\<=9.4.58, \<=10.0.26, \<=11.0.26, \<=12.0.30, \<=12.1.4
|12.0.31, 12.1.5 (9.4.59, 10.0.27, 11.0.27 see https://webtide.com/end-of-life/[details] for availability)

|Aug 20, 2025
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2025-5115[CVE-2025-5115]
|High
|\<=9.4.57, \<=10.0.25, \<=11.0.25, \<=12.0.24, \<=12.1.0.beta2
|9.4.58, 10.0.26, 11.0.26, 12.0.25, 12.1.0.beta3

|May 8, 2025
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2025-1948[CVE-2025-1948]
|High
|\<=12.0.16
|12.0.17

|May 8, 2025
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-13009[CVE-2024-13009]
|High
|\<=9.4.56
|9.4.57

|May 8, 2025
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-13009[CVE-2024-13009]
|High
|\<=9.4.56
|9.4.57

|Oct 14, 2024
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-6763[CVE-2024-6763]
|Low
|\<=12.0.11
|12.0.12

|Oct 14, 2024
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-8184[CVE-2024-8184]
|Moderate
|\<=9.0.55, \<=10.0.23, \<=11.0.23, \<=12.0.8
|9.4.56, 10.0.24, 11.0.24, 12.0.9

|Oct 14, 2024
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-22201[CVE-2024-22201]
|High
|\<=9.0.53, \<=10.0.19, \<=11.0.19, \<=12.0.5
|9.4.54, 10.0.20, 11.0.20, 12.0.6

|Oct 14, 2024
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-9823[CVE-2024-9823]
|Moderate
|\<=9.0.53, \<=10.0.17, \<=11.0.17, \<=12.0.2
|9.4.54, 10.0.17, 11.0.17, 12.0.3

|Oct 14, 2024
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2024-6762[CVE-2024-6762]
|Moderate
|\<=10.0.17, \<=11.0.17, \<=12.0.3
|10.0.18, 11.0.18, 12.0.4

|Oct 10, 2023
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-36478[CVE-2023-36478]
|High
|\<=9.4.52, \<=10.0.15, \<=11.0.15
|9.4.53, 10.0.16, 11.0.16

|Sep 14, 2023
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-41900[CVE-2023-41900]
|Low
|\<=9.4.51, \<=10.0.15, \<=11.0.15
|9.4.52, 10.0.16, 11.0.16

|Sep 14, 2023
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-40167[CVE-2023-40167]
|Low
|\<=9.4.51, \<=10.0.15, \<=11.0.15, \<=12.0.0
|9.4.52, 10.0.16, 11.0.16, 12.0.1

|Sep 14, 2023
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-36479[CVE-2023-36479]
|Low
|\<=9.4.51, \<=10.0.15, \<=11.0.15, \<=12.0.0.beta1
|9.4.52, 10.0.16, 11.0.16, 12.0.0.beta2

|Jul 10, 2023
|https://github.com/jetty/jetty.project/security/advisories/GHSA-58qw-p7qm-5rvh[GHSA-58qw-p7qm-5rvh]
|Low
|\<=9.4.51, \<=10.0.15, \<=11.0.15, \<=12.0.beta4
|9.4.52, 10.0.16, 11.0.16, 12.0.0

|Apr 18, 2023
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-26049[CVE-2023-26049]
|Low
|\<=9.4.50, \<=10.0.13, \<=11.0.13, \<=12.0.0.alpha3
|9.4.51, 10.0.14, 11.0.14, 12.0.0.beta0

|Apr 18, 2023
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-26048[CVE-2023-26048]
|Med
|\<=9.4.50, \<=10.0.13, \<=11.0.13
|9.4.51, 10.0.14, 11.0.14

|Jul 7, 2022
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-2191[CVE-2022-2191]
|High
|\<=10.0.9, \<=11.0.9
|10.0.10, 11.0.10

|Jul 7, 2022
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-2047[CVE-2022-2047]
|Low
|\<=9.4.46, \<=10.0.9, \<=11.0.9
|9.4.47, 10.0.10, 11.0.10

|Jul 7, 2022
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-2048[CVE-2022-2048]
|High
|\<=9.4.46, \<=10.0.9, <=11.0.9
|9.4.47, 10.0.10, 11.0.10

|Jul 15, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-34429[CVE-2021-34429]
|Med
|\<=9.4.42, \<=10.0.5, \<=11.0.5
|9.4.43, 10.0.6, 11.0.6

|Jun 22, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-34428[CVE-2021-34428]
|Low
|\<=9.4.40, \<=10.0.2, \<=11.0.2
|9.4.41, 10.0.3, 11.0.3

|Jun 8, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-28169[CVE-2021-28169]
|Med
|\<=9.4.40, \<=10.0.2, \<=11.0.2
|9.4.41, 10.0.3, 11.0.3

|Apr 1, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-28165[CVE-2021-28165]
|High
|\<=9.4.38, \<=10.0.1, \<=11.0.1
|9.4.39, 10.0.2, 11.0.2

|Apr 1, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-28164[CVE-2021-28164]
|Med
|\<=9.4.38
|9.4.39

|Apr 1, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-28163[CVE-2021-28163]
|Med
|\<=9.4.38, \<=10.0.1, \<=11.0.1
|9.4.39, 10.0.2, 11.0.2

|Feb 26, 2021
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-27223[CVE-2020-27223]
|Med
|\<=9.4.36, \<=10.0.0, \<=11.0.0
|9.4.37, 10.0.1, 11.0.1

|Nov 27, 2020
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-27218[CVE-2020-27218]
|Med
|\<=9.4.34, \<=10.0.0.beta2, \<=11.0.0.beta2
|9.4.35, 10.0.0.beta3, 11.0.0.beta3

|Oct 22, 2020
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-27216[CVE-2020-27216]
|High
|\<=9.4.32
|9.3.29, 9.4.33

|Jul 9, 2020
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-17638[CVE-2019-17638]
|High
|\<=9.4.29
|9.4.30

|Nov 25, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-17632[CVE-2019-17632]
|Med
|\<=9.4.23
|9.4.24

|Aug 13, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9518[CVE-2019-9518]
|Med
|\<=9.4.20
|9.4.21

|Aug 13, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9516[CVE-2019-9516]
|Med
|\<=9.4.20
|9.4.21

|Aug 13, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9515[CVE-2019-9515]
|Med
|\<=9.4.20
|9.4.21

|Aug 13, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9514[CVE-2019-9514]
|Med
|\<=9.4.20
|9.4.21

|Aug 13, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9512[CVE-2019-9512]
|Low
|\<=9.4.20
|9.4.21

|Aug 13, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9511[CVE-2019-9511]
|Low
|\<=9.4.20
|9.4.21

|Apr 22, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-10247[CVE-2019-10247]
|Med
|\<=9.4.16
|9.2.28, 9.3.27, 9.4.17

|Apr 22, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-10246[CVE-2019-10246]
|High
|\<=9.4.16
|9.2.28, 9.3.27, 9.4.17

|Apr 22, 2019
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-10241[CVE-2019-10241]
|High
|\<=9.4.15
|9.2.27, 9.3.26, 9.4.16

|Jun 22, 2018
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2018-12538[CVE-2018-12538]
|High
|\<=9.4.8
|9.4.9

|Jun 26, 2018
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2018-12536[CVE-2018-12536]
|See https://cwe.mitre.org/data/definitions/209.html[CWE-202]
|\<=9.4.10
|9.2.25, 9.3.24, 9.4.11

|Jun 26, 2018
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2017-7658[CVE-2017-7658]
|See https://cwe.mitre.org/data/definitions/444.html[CWE-444]
|\<=9.4.10
|9.2.25, 9.3.24, 9.4.11

|Jun 26, 2018
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2017-7657[CVE-2017-7657]
|See https://cwe.mitre.org/data/definitions/444.html[CWE-444]
|\<=9.4.10
|9.2.25, 9.3.24, 9.4.11

|Jun 26, 2018
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2017-7656[CVE-2017-7656]
|See https://cwe.mitre.org/data/definitions/444.html[CWE-444]
|\<=9.4.10
|9.2.25, 9.3.24, 9.4.11

|Apr 13, 2017
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2016-4800[CVE-2016-4800]
|High
|\<=9.3.8
|9.3.9

|Oct 7, 2016
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2015-2080[CVE-2015-2080]
|High
|\<=9.2.8
|9.2.9

|Dec 30, 2011
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2011-4461[CVE-2011-4461]
|Medium
|\<=8.1.0.RC2
|7.6.0.RCO

|Jan 8, 2008
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2007-6672[CVE-2007-6672]
|Medium
|\<=6.1.6
|6.1.7

|Dec 5, 2007
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2007-5614[CVE-2007-5614]
|Low
|\<=6.1.5
|6.1.6

|Dec 5, 2007
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2007-5613[CVE-2007-5613]
|Low
|\<=6.1.5
|6.1.6

|Dec 5, 2007
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2007-5615[CVE-2007-5615]
|Medium
|\<=6.1.5
|6.1.6rc0

|Feb 7, 2007
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2006-6969[CVE-2006-6969]
|High
|\<=6.0.1, \<=5.1.11, \<=4.2.26
|6.1.0, 6.0.2, 5.1.12, 4.2.27

|Jun 2, 2006
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2006-2759[CVE-2006-2759]
|Medium
|\<=6.0.0Beta16
|6.0.0Beta17

|Jun 2, 2006
|https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2006-2758[CVE-2006-2758]
|Medium
|\<=5.1.5
|5.1.6, 6.0.0Beta4

|===
