...
module.exports = (dest, bundleName, owner, repo, dir, ref, token, updateBranch, latestAlias) => async () => {
  ...
  tree = await octokit.git
    .createTree({
      owner,
      repo,
      tree: [{ path: ospath.posix.join(dir || '', 'README.adoc'), mode: '100644', type: 'blob', sha: readmeBlob }],
      base_tree: tree,
    })
    .then((result) => result.data.sha)
